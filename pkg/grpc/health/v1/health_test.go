...
func TestProbezDefaultStatus(t *testing.T) {
	...
	require.Equal(t, HealthCheckResponse_UNKNOWN, srv.ServiceStatus("", false))
	require.Equal(t, HealthCheckResponse_SERVICE_UNKNOWN, srv.ServiceStatus("", true))
	require.Equal(t, srv.ServiceStatus(DefaultService, false), srv.ServiceStatus("", false))
	require.Equal(t, srv.ServiceStatus(DefaultService, true), srv.ServiceStatus("", true))

	// Mark default service as healthy
	srv.Healthy()
	require.Equal(t, StatusServing, srv.ServiceStatus("", false))
	require.Equal(t, StatusServing, srv.ServiceStatus("", true))
	require.Equal(t, srv.ServiceStatus(DefaultService, false), srv.ServiceStatus("", false))
	require.Equal(t, srv.ServiceStatus(DefaultService, true), srv.ServiceStatus("", true))

	// Mark default service as not healthy
	srv.NotHealthy()
	require.Equal(t, StatusNotServing, srv.ServiceStatus("", false))
	require.Equal(t, StatusNotServing, srv.ServiceStatus("", true))
	require.Equal(t, srv.ServiceStatus(DefaultService, false), srv.ServiceStatus("", false))
	require.Equal(t, srv.ServiceStatus(DefaultService, true), srv.ServiceStatus("", true))
}
...
func TestProbezServiceStatus(t *testing.T) {
	...
	for i, service := range services {
		...
		require.Equal(t, HealthCheckResponse_UNKNOWN, srv.ServiceStatus(service, false))
		require.Equal(t, HealthCheckResponse_SERVICE_UNKNOWN, srv.ServiceStatus(service, true))

		srv.SetStatus(service, StatusNotServing)
		require.Equal(t, StatusNotServing, srv.ServiceStatus(service, false))
		require.Equal(t, StatusNotServing, srv.ServiceStatus(service, true))
		...
		for j, otherService := range services {
			...
			if j < i {
				require.Equal(t, StatusServing, srv.ServiceStatus(otherService, false))
				require.Equal(t, StatusServing, srv.ServiceStatus(otherService, true))
			} else {
				require.Equal(t, HealthCheckResponse_UNKNOWN, srv.ServiceStatus(otherService, false))
				require.Equal(t, HealthCheckResponse_SERVICE_UNKNOWN, srv.ServiceStatus(otherService, true))
			}
		}

		srv.SetStatus(service, StatusServing)
		require.Equal(t, StatusServing, srv.ServiceStatus(service, false))
		require.Equal(t, StatusServing, srv.ServiceStatus(service, true))
...
func TestServer(t *testing.T) {
	...
	for _, service := range []string{"", DefaultService, "otter.v1.Otter"} {
	...
	probe.NotHealthy()
	probe.SetStatus("otter.v1.Otter", StatusNotServing)
	...
	probe.Healthy()
	probe.SetStatus("otter.v1.Otter", StatusServing)
