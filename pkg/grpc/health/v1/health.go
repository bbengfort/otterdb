...
func (h *ProbeServer) ServiceStatus(service string, stream bool) (status HealthCheckResponse_ServingStatus) {
	var ok bool
	if service == "" {
		service = DefaultService
	}
...
func (h *ProbeServer) Check(ctx context.Context, in *HealthCheckRequest) (out *HealthCheckResponse, err error) {
	out = &HealthCheckResponse{
		Status: h.ServiceStatus(in.Service, false),
	}
	return out, nil
}
...
func (h *ProbeServer) Watch(in *HealthCheckRequest, stream Health_WatchServer) (err error) {
	...
	if err = stream.Send(&HealthCheckResponse{Status: h.ServiceStatus(in.Service, true)}); err != nil {
