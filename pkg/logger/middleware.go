...
func GinLogger(server string) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		case 1:
			msg = fmt.Sprintf("%s %s %s %d [error]", server, c.Request.Method, c.Request.URL.Path, status)
		default:
			msg = fmt.Sprintf("%s %s %s %d [%d errors]", server, c.Request.Method, c.Request.URL.Path, status, len(c.Errors))
		}
