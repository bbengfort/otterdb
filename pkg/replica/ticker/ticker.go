...
func New(interval Interval) *Ticker {
	return NewWithContext(context.Background(), interval)
}
...
func NewWithContext(ctx context.Context, interval Interval) *Ticker {
...
type Ticker struct {
	C         <-chan time.Time // The channel on which the events are delivered.
	interval  Interval
...
func (t *Ticker) run(ctx context.Context, c chan<- time.Time) {
